...
- **Retail-style tooltips.** The beige border gives way to a thin grey one with rounded corners like retail's, flush with the tooltip's edge, on unit, item and comparison tooltips alike. A player's name is in their class colour, with the guild in grey brackets, race and class on the level line and the faction in Alliance blue or Horde red. A unit's health bar sits inside the bottom of its tooltip in an outlined track, in class or reaction colour, with health in the tooltip's own small shadowed font, as numbers for players and a percentage for others. A comparison tooltip's Equipped label moves inside its top in grey. On by default; turn it off under Interface.
