...
- **Maps**: reveal unexplored areas on zone maps, tinted so explored ground still stands out. Pointing at a zone on the world map adds its level range after the name, coloured like quest levels for your character. Every dungeon and raid entrance gets retail's icon on zone and continent maps, named when you point at it.
- **Interface**: move and scale Blizzard windows from a Windows tab in Edit Mode, per layout. Quiet the errors a spammed macro repeats. The quest tracker sorts nearest first, with distances. Campsite features say exactly what they give and whether you have it, and a small panel by a campfire lists every camp benefit before you sit down. The reagent bag joins the bottom of the combined bag and opens with it. The spellbook shows how many spells you can train now and what comes at which level. Tooltips look like retail's: a thin rounded grey border, player names in class colour with race and class on one line, the health bar inside with numbers or a percentage, and the Equipped label inside comparison tooltips. Nameplates are larger and clearer: a taller bar with the name and level above it, your target in gold and the rest faded back, and casts in a slim bar with the spell inside. `/rl` reloads.
