Tweaks Forever replaces a handful of single-purpose addons with options in WoW: Forever's own settings: quest and gossip automation, repairs, junk selling, gear groups, a revealed world map, movable windows and fishing. Nothing opens a window of its own apart from a small panel of camp benefits by a campfire, and when another addon already does one of these jobs, that option is greyed out with the addon named, so the two never fight.
...
![Kalimdor on the world map with a small level range at each zone, coloured for a level 22 character: grey 1-10 starting zones, yellow 10-20 Darkshore and 18-30 Ashenvale, orange and red for the zones above](https://raw.githubusercontent.com/cjber/tweaks-forever/main/docs/screenshots/zonelevels.png)
...
![The Camp panel: a hint to sit or craft near a camp feature for a minute, three benefits in green with their time left, such as First Aid Kit (52 Min): Stamina increased by 56, and the seven others in grey with what they would give](https://raw.githubusercontent.com/cjber/tweaks-forever/main/docs/screenshots/camp.png)

## Features

- **Automation**, all off until you turn it on: accept and turn in quests (hold Shift to talk normally), skip single-option gossip, accept summons, accept resurrections from your group (never mid-combat), release in battlegrounds, decline duels. Faster auto loot is on by default.
...
- **Maps**: reveal unexplored areas on zone maps, tinted so explored ground still stands out. Each zone's level range shows on the continent maps, coloured like quest levels for your character.
- **Interface**: move and scale Blizzard windows from a Windows tab in Edit Mode, per layout. Quiet the errors a spammed macro repeats. The quest tracker sorts nearest first, with distances. Campsite features say exactly what they give and whether you have it, and a small panel by a campfire lists every camp benefit before you sit down. The reagent bag joins the bottom of the combined bag and opens with it. The spellbook shows how many spells you can train now and what comes at which level. Tooltips look like retail's: a thin rounded grey border, player names in class colour with race and class on one line, the health bar inside with numbers or a percentage, and the Equipped label inside comparison tooltips. `/rl` reloads.
...
It works alongside Leatrix Plus, Questie, BlizzMove, Peddler, FishingBuddy and others: the matching option steps aside while they are loaded.

Source code and issues: [github.com/cjber/tweaks-forever](https://github.com/cjber/tweaks-forever) (GPL-3.0).
