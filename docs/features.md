...
- Quests on minimap givers. With QuestieDB installed, pointing at a quest giver on the minimap lists the quests they have for you, such as "[15] Dry Times", coloured by level, and the next few levels' in grey with the level they open at. A giver you owe a quest to lists it with a gold ? once it is ready to hand in.
- Quest progress on tooltips. Pointing at a creature you need to kill for a quest in your log adds the quest's title and your progress under its tooltip, as the retail game does: " - Mangy Wolf slain: 3/10", in white until that objective is done and grey after. The quest log is enough for kills it writes in its own words. With QuestieDB installed, creatures that drop an item a quest wants count too, and so do kills the quest describes in words of its own. On by default.
- Quest icons on nameplates. A small gold quest icon sits to the right of the nameplate of each creature you still need, and goes once you have enough or the quest is complete. The same creatures count as for the tooltips. On by default.
