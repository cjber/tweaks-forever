...
- Quest progress on tooltips. With QuestieDB installed, pointing at a creature you need for a quest in your log adds the quest's title and your progress under its tooltip, as the retail game does: " - Mangy Wolf slain: 3/10", in white until that objective is done and grey after. A creature that drops an item a quest wants counts too, with the item's count. On by default.
- Quest icons on nameplates. With QuestieDB installed, a small gold quest icon sits to the right of the nameplate of each creature you still need, and goes once you have enough or the quest is complete. On by default.
