...
local renames = Model.Renames({ "preset:1", first, second }, { "preset:1", renamed, second })
assert(renames[first] == renamed and next(renames, first) == nil, "a rename is the one changed key")
assert(next(Model.Renames({ "preset:1", first }, { "preset:1", first, second })) == nil, "an added layout is no rename")
assert(next(Model.Renames({ "preset:1", first, second }, { "preset:1", second })) == nil, "nor is a deleted one")
