...
def resolve(spells, text, own):
    """Substitute $[spell]w<n>, $[spell]t<n> and $[spell]d; anything else left is a failure."""
    ...
    def value(match):
        ...
        return number(row["EffectBasePointsF"]) if kind == "w" else number(int(row["EffectAuraPeriod"]) / 1000)
...
def benefits(spells):
    ...
    for aura, body in branches:
        ...
        if aura == TENT_AURA:
            ...
            out.append((aura, feature, resolve(spells, spells.description(TENT), TENT), None))
        else:
            out.append((aura, feature, resolve(spells, effect, aura).rstrip("."), spells.seconds(aura)))
    return out
...
def render(rows):
    ...
    for aura, feature, effect, seconds in rows:
