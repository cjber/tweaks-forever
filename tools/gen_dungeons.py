...
The public API keeps each instance's first curated zone projection before any pins are merged.
...
def generate(tables):
    ...
    for instance, row in sorted(instances.items()):
        ...
        for ui_map in (*ZONES[instance], *sorted(continents)):
            ...
            if at:
                if ui_map == ZONES[instance][0]:
                    entrances[instance] = (ui_map, *at)
    ...
    return entrances, pins, raids, {i: r["MapName_lang"] for i, r in instances.items()}, ui_maps, unplaced


def render(entrances, pins, raids, names, ui_maps, areas):
    lines = [
    ...
    for instance, (ui_map, x, y) in entrances.items():
        lines.append(f"\t[{instance}] = {{ map = {ui_map}, x = {x}, y = {y} }}, -- {names[instance]}")
    lines += ["}", "ns.DungeonEntrances = {"]
...
def main():
    ...
    entrances, pins, raids, instance_names, ui_maps, unplaced = generate(tables)
    areas = {int(r["ID"]): r["AreaName_lang"] for r in tables["AreaTable"]}
    OUTPUT.write_text(render(entrances, pins, raids, instance_names, ui_maps, areas), encoding="utf-8")
